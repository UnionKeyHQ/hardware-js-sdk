...
> **Note:**
> Demo: Native iOS example (WKWebView + CoreBluetooth) → [native-ios-example](https://github.com/UnionKeyHQ/hardware-js-sdk/tree/onekey/packages/connect-examples/native-ios-example)
> This guide builds on the [Low-Level Transport Plugin](/en/hardware-sdk/concepts/low-level-plugin) and its 64-byte message protocol—read that first.


This page shows how to integrate `@unionkeyhq/hd-common-connect-sdk` in a native iOS host via a low-level adapter. The JavaScript bundle runs in WKWebView; transport calls are forwarded to native CoreBluetooth and bridged back to JS.
...
- Message Protocol (64‑byte framing): [See Low-Level Transport Plugin](/en/hardware-sdk/concepts/low-level-plugin#message-protocol-part-of-low-level-transport)
- Low‑level transport plugin contract: [Low-level Transport Plugin](/en/hardware-sdk/concepts/low-level-plugin)
