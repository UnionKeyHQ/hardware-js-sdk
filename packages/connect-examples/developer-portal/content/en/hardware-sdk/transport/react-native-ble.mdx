...
This guide shows how to use the pure React Native BLE stack with `@unionkeyfe/hd-ble-sdk` and `@unionkeyfe/hd-transport-react-native`. No WebView or low-level adapter is required. The chain API usage stays the same as in Quick Start.
...
```bash
npm i @unionkeyfe/hd-ble-sdk @unionkeyfe/hd-transport-react-native \
      @unionkeyfe/react-native-ble-utils react-native-ble-plx \
