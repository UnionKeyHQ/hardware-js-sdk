...
> **Note:**
Demo: Playground (supports emulator) → [hardware-example.unionkey.io](https://hardware-example.unionkey.io/)


Use this step‑by‑step guide to wire WebUSB in the browser with `@unionkeyhq/hd-common-connect-sdk`. Once connected, you can call any chain API (BTC/EVM/etc.) as in Quick Start.
...
- A connected and unlocked UnionKey device

## Step 2. Install

```bash
npm i @unionkeyhq/hd-common-connect-sdk @unionkeyhq/hd-shared @unionkeyhq/hd-core
```
...
- PIN: prefer on-device entry (`@@UNIONKEY_INPUT_PIN_IN_DEVICE`). If you provide software input, use the blind keypad mapping (`7,8,9,4,5,6,1,2,3`).
...
Use the UNIONKEY_WEBUSB_FILTER filters so the chooser only shows supported devices.
