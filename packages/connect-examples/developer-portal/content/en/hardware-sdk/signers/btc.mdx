...
UnionKey BTC signer talks to the device app via `HardwareSDK`, wraps APDUs into UTXO review and signing tasks.
...
- Run calls serially per device; use `showOnUnionKey` to confirm receive/change paths before signing.
...
await HardwareSDK.btcGetAddress(connectId, deviceId, { path: path49, coin: 'btc', showOnUnionKey: true });
