...
| --- | --- | --- | --- |
| `connectId` | Routes calls to a specific device/connection | `searchDevices()` | A new connection opens (USB re-plug, BLE pairing, transport switch). |
| `deviceId` | Identifies the currently-loaded seed / wallet state | `getFeatures(connectId)` | Factory reset, wipe, seed recovery, or seed switch. |
...
`connectId` format varies per transport (WebUSB = device serial, Android BLE = MAC, iOS BLE = CoreBluetooth UUID, Desktop BLE = hex UUID/MAC, …). See [Identifiers → `connectId`](/en/hardware-sdk/concepts/identifiers#connectid) for the full per-transport table and worked examples.
...
```ts
import HardwareSDK, { UI_EVENT, UI_REQUEST, UI_RESPONSE } from '@unionkeyfe/hd-common-connect-sdk';
...
More detail: see [`HardwareError.ts`](https://github.com/UnionKeyHQ/hardware-js-sdk/blob/onekey/packages/shared/src/HardwareError.ts) for exhaustive definitions and payload shapes.
