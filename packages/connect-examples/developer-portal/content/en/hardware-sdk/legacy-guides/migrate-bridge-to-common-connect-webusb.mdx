...
Migrate from Bridge to WebUSB and switch SDK entry to `@unionkeyfe/hd-common-connect-sdk`.
...
- SDK entry: `@unionkeyfe/hd-web-sdk` → `@unionkeyfe/hd-common-connect-sdk`
...
- Event system: use `@unionkeyfe/hd-core` `UI_EVENT / DEVICE` events for user interaction (PIN/Passphrase, etc.)
...
```bash
npm i @unionkeyfe/hd-common-connect-sdk @unionkeyfe/hd-core @unionkeyfe/hd-shared
```
...
```ts
import { UI_EVENT, UI_REQUEST, UI_RESPONSE, DEVICE } from '@unionkeyfe/hd-core';
...
```ts
import { UNIONKEY_WEBUSB_FILTER } from '@unionkeyfe/hd-shared';
...
- Dependency swap: remove `@unionkeyfe/hd-web-sdk`, use `@unionkeyfe/hd-common-connect-sdk` instead
