...
Replace Bridge with WebUSB while keeping `@unionkeyfe/hd-web-sdk` as your SDK entry.
...
- Update hd-web-sdk `connectSrc` to the latest iframe host version (e.g., `https://jssdk.unionkey.io/1.1.16/`)
- Ask for user authorization via `navigator.usb.requestDevice()` with official UnionKey filters — must be triggered by a user gesture (e.g., a button click) due to Chrome security restrictions; automatic device discovery is not allowed before permission
...
```ts
import HdWebSdk from '@unionkeyfe/hd-web-sdk';
...
await HardwareSDK.init({
  ...
  connectSrc: 'https://jssdk.unionkey.io/1.1.16/',
  debug: true,
  fetchConfig: true,
});
```
