---
title: Conflux
description: Conflux integration via UnionKey's Fluent-compatible provider
---
...
Integrate Conflux blockchain using UnionKey's Fluent-compatible provider. Access via `window.$onekey.conflux`.

<Callout type="info">
UnionKey's Conflux provider is compatible with both ConfluxPortal and Fluent wallet interfaces.
</Callout>
...
```typescript
// Detect UnionKey Conflux provider
const provider = window.$onekey?.conflux

if (!provider) {
  throw new Error('UnionKey Conflux provider not detected')
}
...
console.log('isUnionKey:', provider.isUnionKey) // true
```
...
// Use UnionKey provider for signing
