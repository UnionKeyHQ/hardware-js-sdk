---
title: Sui
description: Sui integration via UnionKey's Sui Wallet Standard provider
---
...
Integrate Sui blockchain using UnionKey's provider. Access via `window.$onekey.sui`.

<Callout type="info">
UnionKey implements the Sui Wallet Standard, ensuring compatibility with all Sui dApps.
</Callout>
...
```typescript
// Detect UnionKey Sui provider
const provider = window.$onekey?.sui

if (!provider) {
  throw new Error('UnionKey Sui provider not detected')
}
```
...
UnionKey is automatically detected through the Sui Wallet Standard.
