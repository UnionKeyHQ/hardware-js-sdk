---
title: Cosmos
description: Cosmos integration via UnionKey's Keplr-compatible provider
---
...
Integrate Cosmos ecosystem chains using UnionKey's Keplr-compatible provider. Access via `window.$onekey.cosmos`.

<Callout type="info">
UnionKey's Cosmos provider is fully compatible with the Keplr wallet interface, making migration seamless.
</Callout>
...
```typescript
// Detect UnionKey Cosmos provider
const provider = window.$onekey?.cosmos

if (!provider) {
  throw new Error('UnionKey Cosmos provider not detected')
}

// Check if it's UnionKey
...
UnionKey's Cosmos provider is Keplr-compatible. Simply change your provider detection:

```typescript
// Before (Keplr only)
const provider = window.keplr

// After (UnionKey with Keplr fallback)
