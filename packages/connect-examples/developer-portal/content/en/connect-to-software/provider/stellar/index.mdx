---
title: Stellar
description: Stellar integration via UnionKey's Stellar provider
---
...
Integrate Stellar using UnionKey's Stellar provider. Access via `window.$onekey.stellar`.
...
if (!stellar?.isUnionKey) throw new Error('UnionKey Stellar provider not detected')
