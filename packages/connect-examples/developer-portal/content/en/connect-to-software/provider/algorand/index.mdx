---
title: Algorand
description: Algorand integration via UnionKey's provider
---
...
Integrate Algorand blockchain using UnionKey's provider. Access via `window.$onekey.algo`.

<Callout type="info">
UnionKey's Algorand provider supports both legacy and ARC-0001 compliant interfaces.
</Callout>
...
```typescript
// Detect UnionKey Algorand provider
const provider = window.$onekey?.algo

if (!provider) {
  throw new Error('UnionKey Algorand provider not detected')
}
...
| `isUnionKey` | `boolean` | UnionKey identifier |
