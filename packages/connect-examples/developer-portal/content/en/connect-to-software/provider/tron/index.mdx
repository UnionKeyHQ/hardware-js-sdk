---
title: TRON
description: TRON integration via UnionKey's TronLink-compatible provider
---
...
Integrate TRON blockchain using UnionKey's TronLink-compatible provider. Access via `window.$onekey.tron` or `window.tronWeb`.

<Callout type="info">
UnionKey's TRON provider is fully compatible with TronLink, including TronWeb and SunWeb instances.
</Callout>
...
```typescript
// UnionKey injects both the provider and TronWeb instances
...
if (!provider) {
  throw new Error('UnionKey TRON provider not detected')
}
...
UnionKey is TronLink-compatible. Update your detection logic:

```typescript
// Before (TronLink only)
const tronWeb = window.tronWeb

// After (UnionKey with TronLink fallback)
