---
title: TON
description: TON integration via UnionKey's TON Connect compatible provider
---
...
Integrate TON blockchain using UnionKey's TON Connect compatible provider. Access via `window.$onekey.tonconnect`.

<Callout type="info">
UnionKey implements the TON Connect 2.0 protocol, ensuring compatibility with all TON dApps.
</Callout>
...
```typescript
// Detect UnionKey TON provider
const provider = window.$onekey?.tonconnect

if (!provider) {
  throw new Error('UnionKey TON provider not detected')
}
...
UnionKey is automatically detected by TON Connect SDK.
