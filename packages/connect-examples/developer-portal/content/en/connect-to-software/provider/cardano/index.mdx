---
title: Cardano
description: Cardano integration via UnionKey's CIP-30 compatible provider
---
...
Integrate Cardano blockchain using UnionKey's CIP-30 compatible provider. Access via `window.cardano.onekey` or `window.cardano.nami`.

<Callout type="info">
UnionKey implements CIP-30 (Cardano dApp Connector), ensuring compatibility with all Cardano dApps. It also provides Nami wallet compatibility.
</Callout>
...
```typescript
// UnionKey provides both onekey and nami interfaces
const onekey = window.cardano?.onekey
const nami = window.cardano?.nami

// Check availability
if (!onekey) {
  throw new Error('UnionKey Cardano provider not detected')
}

// Get wallet info
console.log('Name:', onekey.name) // 'UnionKey'
...
// Initialize Lucid with UnionKey
...
UnionKey provides Nami wallet compatibility:
