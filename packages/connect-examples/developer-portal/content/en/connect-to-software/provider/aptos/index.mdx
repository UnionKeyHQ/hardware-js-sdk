---
title: Aptos
description: Aptos integration via UnionKey's Petra-compatible provider
---
...
Integrate Aptos blockchain using UnionKey's Petra-compatible provider. Access via `window.$onekey.aptos`.

<Callout type="info">
UnionKey's Aptos provider is fully compatible with the Petra wallet interface and Aptos Wallet Standard.
</Callout>
...
```typescript
// Detect UnionKey Aptos provider
const provider = window.$onekey?.aptos

if (!provider) {
  throw new Error('UnionKey Aptos provider not detected')
}

// Check provider info
console.log('Provider:', provider.isUnionKey ? 'UnionKey' : 'Unknown')
```
...
UnionKey is automatically detected by the Aptos Wallet Adapter.

---

## Migration from Petra

UnionKey's Aptos provider is Petra-compatible. Update your provider detection:

```typescript
// Before (Petra only)
const provider = window.petra

// After (UnionKey with Petra fallback)
