...
In order to start interacting with UnionKey you must first establish a connection. This connection request will prompt the user for permission to share their account id and public key, and indicate that they are willing to interact further.
...
The **recommended** and **easiest** way to connect to UnionKey is by calling `provider.requestSignIn()` . However, the provider also exposes a `request` JSON RPC interface.
