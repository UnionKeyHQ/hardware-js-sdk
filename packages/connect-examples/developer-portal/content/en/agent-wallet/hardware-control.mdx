...
Hardware control lets an Agent Wallet workflow escalate sensitive operations to a UnionKey hardware wallet. This is UnionKey's core difference from agent wallets that only rely on software custody or TEE signing.

## When To Escalate

| Flow | Hardware expectation |
| --- | --- |
| High-value transfer | User reviews recipient, amount, chain, and fee on the UnionKey device. |
...
| `walletKind` | `hw` |
| `device` | Connected UnionKey device info |
