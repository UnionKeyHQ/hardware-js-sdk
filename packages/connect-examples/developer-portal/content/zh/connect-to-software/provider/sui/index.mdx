---
title: Sui
description: 通过 UnionKey 的 Sui Wallet Standard Provider 集成 Sui
---
...
使用 UnionKey 的 Provider 集成 Sui 区块链。通过 `window.$onekey.sui` 访问。

<Callout type="info">
UnionKey 实现了 Sui Wallet Standard，确保与所有 Sui dApp 兼容。
</Callout>
...
```typescript
// 检测 UnionKey Sui provider
const provider = window.$onekey?.sui

if (!provider) {
  throw new Error('未检测到 UnionKey Sui provider')
}
```
...
UnionKey 会通过 Sui Wallet Standard 自动被检测到。
