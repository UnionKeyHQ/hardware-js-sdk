---
title: Ethereum / EVM
description: 使用 UnionKey 的 EIP-1193 Provider 进行 Ethereum 和 EVM 集成
---
...
使用 UnionKey 注入的 EIP-1193 Provider 构建 EVM dApp。所有 EVM 兼容链（Ethereum、Polygon、BSC、Arbitrum、Optimism、Base 等）共享同一个 Provider。
...
if (!provider) throw new Error('未检测到 UnionKey')
