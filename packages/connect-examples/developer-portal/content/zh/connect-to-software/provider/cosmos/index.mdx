---
title: Cosmos
description: 通过 UnionKey 的 Keplr 兼容 Provider 集成 Cosmos
---
...
使用 UnionKey 的 Keplr 兼容 Provider 集成 Cosmos 生态系统链。通过 `window.$onekey.cosmos` 访问。

<Callout type="info">
UnionKey 的 Cosmos Provider 完全兼容 Keplr 钱包接口，迁移无缝衔接。
</Callout>
...
```typescript
// 检测 UnionKey Cosmos provider
const provider = window.$onekey?.cosmos

if (!provider) {
  throw new Error('未检测到 UnionKey Cosmos provider')
}
...
UnionKey 的 Cosmos provider 兼容 Keplr。只需更改 provider 检测代码：

```typescript
// 之前 (仅 Keplr)
const provider = window.keplr

// 之后 (UnionKey 优先，Keplr 回退)
