---
title: TON
description: 通过 UnionKey 的 TON Connect 兼容 Provider 集成 TON
---
...
使用 UnionKey 的 TON Connect 兼容 Provider 集成 TON 区块链。通过 `window.$onekey.tonconnect` 访问。

<Callout type="info">
UnionKey 实现了 TON Connect 2.0 协议，确保与所有 TON dApp 兼容。
</Callout>
...
```typescript
// 检测 UnionKey TON provider
const provider = window.$onekey?.tonconnect

if (!provider) {
  throw new Error('未检测到 UnionKey TON provider')
}
...
UnionKey 会被 TON Connect SDK 自动检测到。
