---
title: Algorand
description: 通过 UnionKey 的 Provider 集成 Algorand
---
...
使用 UnionKey 的 Provider 集成 Algorand 区块链。通过 `window.$onekey.algo` 访问。

<Callout type="info">
UnionKey 的 Algorand Provider 支持传统接口和 ARC-0001 兼容接口。
</Callout>
...
```typescript
// 检测 UnionKey Algorand provider
const provider = window.$onekey?.algo

if (!provider) {
  throw new Error('未检测到 UnionKey Algorand provider')
}
...
| `isUnionKey` | `boolean` | UnionKey 标识 |
