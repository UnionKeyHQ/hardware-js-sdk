---
title: Cardano
description: 通过 UnionKey 的 CIP-30 兼容 Provider 集成 Cardano
---
...
使用 UnionKey 的 CIP-30 兼容 Provider 集成 Cardano 区块链。通过 `window.cardano.onekey` 或 `window.cardano.nami` 访问。

<Callout type="info">
UnionKey 实现了 CIP-30 (Cardano dApp Connector)，确保与所有 Cardano dApp 兼容。同时提供 Nami 钱包兼容性。
</Callout>
...
```typescript
// UnionKey 提供 onekey 和 nami 两种接口
const onekey = window.cardano?.onekey
const nami = window.cardano?.nami

// 检查可用性
if (!onekey) {
  throw new Error('未检测到 UnionKey Cardano provider')
}

// 获取钱包信息
console.log('名称:', onekey.name) // 'UnionKey'
