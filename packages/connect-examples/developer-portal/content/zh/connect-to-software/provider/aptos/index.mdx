---
title: Aptos
description: 通过 UnionKey 的 Petra 兼容 Provider 集成 Aptos
---
...
使用 UnionKey 的 Petra 兼容 Provider 集成 Aptos 区块链。通过 `window.$onekey.aptos` 访问。

<Callout type="info">
UnionKey 的 Aptos Provider 完全兼容 Petra 钱包接口和 Aptos Wallet Standard。
</Callout>
...
```typescript
// 检测 UnionKey Aptos provider
const provider = window.$onekey?.aptos

if (!provider) {
  throw new Error('未检测到 UnionKey Aptos provider')
}

// 检查 provider 信息
console.log('Provider:', provider.isUnionKey ? 'UnionKey' : '未知')
```
...
UnionKey 会被 Aptos Wallet Adapter 自动检测到。

---

## 从 Petra 迁移

UnionKey 的 Aptos provider 兼容 Petra。更新你的 provider 检测代码：

```typescript
// 之前 (仅 Petra)
const provider = window.petra

// 之后 (UnionKey 优先，Petra 回退)
