---
title: 快速开始
description: 使用 UnionKey 的 Petra 兼容 Provider 连接 Aptos
---
...
UnionKey 的 Aptos provider 与 Petra 兼容。更新您的 provider 检测方式：

```typescript
// 之前 (仅 Petra)
const provider = window.petra

// 之后 (UnionKey 优先，Petra 作为备选)
...
UnionKey 会被 Aptos Wallet Adapter 自动检测。
