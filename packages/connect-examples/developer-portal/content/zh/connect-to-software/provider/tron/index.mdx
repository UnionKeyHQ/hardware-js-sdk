---
title: TRON
description: 通过 UnionKey 的 TronLink 兼容 Provider 集成 TRON
---
...
使用 UnionKey 的 TronLink 兼容 Provider 集成 TRON 区块链。通过 `window.$onekey.tron` 或 `window.tronWeb` 访问。

<Callout type="info">
UnionKey 的 TRON Provider 完全兼容 TronLink，包括 TronWeb 和 SunWeb 实例。
</Callout>
...
```typescript
// UnionKey 注入 provider 和 TronWeb 实例
...
if (!provider) {
  throw new Error('未检测到 UnionKey TRON provider')
}
...
UnionKey 兼容 TronLink。更新你的检测逻辑：

```typescript
// 之前（仅 TronLink）
const tronWeb = window.tronWeb

// 之后（UnionKey 优先，TronLink 回退）
