...
> **注意：**
> 演示：React Native / Expo 示例 → [react-native-demo](https://github.com/UnionKeyHQ/hardware-js-sdk/tree/develop/packages/connect-examples/react-native-demo)


本指南展示如何使用纯 React Native BLE 堆栈与 `@unionkeyhq/hd-ble-sdk` 和 `@unionkeyhq/hd-transport-react-native`。无需 WebView 或底层适配器。链 API 的使用方式与快速开始中相同。
...
- 始终尽早订阅 `UI_EVENT`，以免 PIN/密码短语/确认阻塞请求。参见 [配置事件](/zh/hardware-sdk/core-api-guide#events-ui)。
- 保持 BLE 权限与操作系统版本同步。在物理设备上测试。
- 更多示例请查看：[react-native-demo](https://github.com/UnionKeyHQ/hardware-js-sdk/tree/onekey/packages/connect-examples/react-native-demo)

## 继续

- [简介](/zh/hardware-sdk)
- [错误码](/zh/hardware-sdk/core-api-guide#error-codes)、[通用参数](/zh/hardware-sdk/core-api-guide#common-params)
- 协议帧（64 字节，仅供参考）：[见底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin#消息协议包含在底层传输中)
