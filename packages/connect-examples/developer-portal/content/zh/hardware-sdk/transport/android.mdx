...
> **注意：**
> 演示：原生 Android 示例（WebView + JSBridge + Nordic BLE）→ [native-android-example](https://github.com/UnionKeyHQ/hardware-js-sdk/tree/onekey/packages/connect-examples/native-android-example)
> 本页基于《[底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin)》— 建议先阅读，了解 BLE 消息格式和协议详情。


本指南展示如何通过底层适配器在原生 Android 宿主中集成 `@unionkeyfe/hd-common-connect-sdk`。JavaScript 包在 WebView 中运行；传输调用被转发到原生（Nordic BLE）并桥接回 JS。
...
UnionKey 设备需要安全的 BLE 配对。不配对会导致：
...
UnionKey BLE UUID：参见 [底层传输插件 - 快速参考](/zh/hardware-sdk/concepts/low-level-plugin#快速参考)
...
- 设备上的 PIN：`payload: '@@UNIONKEY_INPUT_PIN_IN_DEVICE'`
