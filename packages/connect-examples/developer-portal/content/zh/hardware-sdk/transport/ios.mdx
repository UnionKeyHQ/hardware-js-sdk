...
> **注意：**
> 演示：原生 iOS 示例（WKWebView + CoreBluetooth）→ [native-ios-example](https://github.com/UnionKeyHQ/hardware-js-sdk/tree/onekey/packages/connect-examples/native-ios-example)
> 本页基于《[底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin)》与消息协议，建议先阅读接口与 64 字节帧格式。


本页面展示如何通过底层适配器在原生 iOS 宿主中集成 `@unionkeyfe/hd-common-connect-sdk`。JavaScript 包在 WKWebView 中运行；传输调用被转发到原生 CoreBluetooth 并桥接回 JS。
...
在 JS 中，您将从适配器调用这些处理程序以镜像演示的行为。否则，您可以完全在 JS 中使用 `UI_EVENT` 处理 UI — 参见 [事件配置](/zh/hardware-sdk/core-api-guide#events-ui) 了解事件连接和响应（WebUSB 指南包含最小对话框）。
...
- 始终在 JS 中订阅 `UI_EVENT` 以避免请求停滞（参见 [事件配置](/zh/hardware-sdk/core-api-guide#events-ui)）。
- 将 `web/web_dist/` 保留在您的应用包中并相应调整 `load` 路径。

## 参考

- 消息协议（64 字节帧）：[见底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin#消息协议包含在底层传输中)
- 底层传输插件契约：[底层传输插件](/zh/hardware-sdk/concepts/low-level-plugin)
