...
处理 UnionKey 设备 Passphrase（隐藏钱包）的基本规则与最佳实践。
...
所有 UnionKey 设备均支持 Passphrase 和会话缓存（`passphraseState`）。
...
> **重要：设备锁定会使会话失效。** 所有 UnionKey 设备在锁定时（无论是自动锁屏、手动锁定还是 USB 断开）都会清除 passphrase 会话。锁定后缓存的 `session_id` 失效，用户必须在解锁（输入 PIN）后重新输入 passphrase。会话复用仅在设备保持解锁状态时有效。
