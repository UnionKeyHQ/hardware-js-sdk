...
将 Bridge 替换为 WebUSB，保持使用 `@unionkeyhq/hd-web-sdk`。
...
- 更新 hd-web-sdk 的 `connectSrc` 到最新 iframe 主机版本（如 `https://jssdk.unionkey.io/1.1.16/`）
- 通过 `navigator.usb.requestDevice()` 配合官方 UnionKey 过滤器请求用户授权 — 由于 Chrome 安全限制，必须由用户手势触发（如按钮点击）；在获得权限前不允许自动设备发现
