...
从 Bridge 迁移到 WebUSB，并切换到 `@unionkeyhq/hd-common-connect-sdk`。
...
- SDK 入口：`@unionkeyhq/hd-web-sdk` → `@unionkeyhq/hd-common-connect-sdk`
...
- 事件系统：使用 `@unionkeyhq/hd-core` 的 `UI_EVENT / DEVICE` 事件处理用户交互（PIN/Passphrase 等）
...
- 依赖替换：移除 `@unionkeyhq/hd-web-sdk`，使用 `@unionkeyhq/hd-common-connect-sdk`
- 初始化变更：
