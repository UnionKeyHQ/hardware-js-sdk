...
从 Bridge 迁移到 WebUSB，并切换到 `@unionkeyfe/hd-common-connect-sdk`。
...
- SDK 入口：`@unionkeyfe/hd-web-sdk` → `@unionkeyfe/hd-common-connect-sdk`
- 初始化选项：使用 `env: 'webusb'`；不再需要 `connectSrc`（无 iframe）
- 权限流程：调用 `navigator.usb.requestDevice()`（必须由用户手势触发）并使用官方过滤器 `UNIONKEY_WEBUSB_FILTER`
- 事件系统：使用 `@unionkeyfe/hd-core` 的 `UI_EVENT / DEVICE` 事件处理用户交互（PIN/Passphrase 等）
...
- 详情参见：[配置事件](/zh/hardware-sdk/core-api-guide#events-ui)
...
- 依赖替换：移除 `@unionkeyfe/hd-web-sdk`，使用 `@unionkeyfe/hd-common-connect-sdk`
- 初始化变更：
  - 旧：`env: 'web'` + `connectSrc: 'https://jssdk.unionkey.io/.../'`
...
- WebUSB 深入解析和完整示例：[WebUSB 连接指南](/zh/hardware-sdk/transport/web-usb)
- 事件和交互：[配置事件](/zh/hardware-sdk/core-api-guide#events-ui)
- API 参考和参数：[Hardware SDK 核心 API 指南](/zh/hardware-sdk/core-api-guide)、[通用参数](/zh/hardware-sdk/core-api-guide#common-params)、[错误码](/zh/hardware-sdk/core-api-guide#error-codes)
