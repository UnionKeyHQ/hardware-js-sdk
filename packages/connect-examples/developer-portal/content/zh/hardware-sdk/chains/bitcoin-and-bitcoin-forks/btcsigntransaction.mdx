...
对预构建交易的指定输入和输出进行签名。用户需要在 UnionKey 上确认所有交易详情。
...
* `coin` - _必需_ `string` 决定 [bitcoin.json](https://github.com/UnionKeyHQ/hardware-js-sdk/blob/onekey/packages/core/src/data/coins/bitcoin.json) 文件中指定的网络定义。可以使用币种的 `shortcut`、`name` 或 `label`。
* `inputs` - _必需_ [TxInputType](https://github.com/UnionKeyHQ/hardware-js-sdk/blob/728279dc70c5bde56c5d72d86598815f977b0c7f/packages/hd-transport/src/types/messages.ts#L363) 的 `Array`
* `outputs` - _必需_ [TxOutputType](https://github.com/UnionKeyHQ/hardware-js-sdk/blob/728279dc70c5bde56c5d72d86598815f977b0c7f/packages/hd-transport/src/types/messages.ts#L390) 的 `Array`
* `refTxs` - _必需_ [RefTransaction](https://github.com/UnionKeyHQ/hardware-js-sdk/blob/728279dc70c5bde56c5d72d86598815f977b0c7f/packages/core/src/types/api/btcSignTransaction.ts#L26) 的 `Array`。Zcash 和 Komodo 的 refTxs 还应包含 `expiry`、`version_group_id` 和 `extra_data` 字段。
