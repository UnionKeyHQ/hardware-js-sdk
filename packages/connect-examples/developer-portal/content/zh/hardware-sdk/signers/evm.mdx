...
EVM 硬件签名全链路指南，基于 @unionkeyhq/hd-core 的能力，覆盖地址确认、EIP‑1559 / Legacy 交易、personal_sign、EIP‑712 等场景。
...
> 本模块依赖 `@unionkeyhq/hd-core` 及连接层（如 `@unionkeyhq/hd-common-connect-sdk`），请先完成基础 SDK 安装。

```bash
npm install @unionkeyhq/hd-core @unionkeyhq/hd-common-connect-sdk
```

## 初始化

```ts
import HardwareSDK from '@unionkeyhq/hd-core';
...
```ts
import HardwareSDK, { UI_REQUEST, UI_RESPONSE } from '@unionkeyhq/hd-core';
