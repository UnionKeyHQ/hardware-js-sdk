...
EVM 硬件签名全链路指南，基于 @unionkeyfe/hd-core 的能力，覆盖地址确认、EIP‑1559 / Legacy 交易、personal_sign、EIP‑712 等场景。
...
EVM 签名通过 `HardwareSDK` 与 UnionKey 硬件 App 通信，内部使用 APDU 完成交互，外部以独立用例暴露（地址、交易、消息、Typed Data）。接口以 Promise `{ success, payload }` 返回结果，交互提示通过 `UI_REQUEST` 事件触发（PIN、密码短语、打开 App、确认地址/交易/消息/Typed Data）。
...
> 本模块依赖 `@unionkeyfe/hd-core` 及连接层（如 `@unionkeyfe/hd-common-connect-sdk`），请先完成基础 SDK 安装。

```bash
npm install @unionkeyfe/hd-core @unionkeyfe/hd-common-connect-sdk
```

## 初始化

```ts
import HardwareSDK from '@unionkeyfe/hd-core';
...
> 保持固件为最新版本，确保 WebUSB/BLE 连接稳定；建议在签名前先完成一次 `evmGetAddress(showOnUnionKey: true)` 以确认路径与账户。
...
- `showOnUnionKey?` (boolean): 可选，是否在设备上展示并让用户确认地址。
...
```ts
import HardwareSDK, { UI_REQUEST, UI_RESPONSE } from '@unionkeyfe/hd-core';
...
await HardwareSDK.evmGetAddress(connectId, deviceId, { path: accountPath, showOnUnionKey: true });
