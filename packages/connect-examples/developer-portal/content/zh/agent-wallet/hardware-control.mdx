...
硬件控制让 Agent Wallet 工作流可以把敏感操作升级到 UnionKey 硬件钱包。这是 UnionKey 和只依赖软件托管或 TEE 签名的 agent wallet 的核心差异。

## 什么时候升级到硬件

| 场景 | 硬件期望 |
| --- | --- |
| 大额转账 | 用户在 UnionKey 设备上复核接收方、金额、链和费用。 |
