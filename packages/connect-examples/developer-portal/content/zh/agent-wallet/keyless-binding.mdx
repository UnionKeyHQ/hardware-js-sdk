...
Keyless 绑定是产品边界：用户可以在 UnionKey GUI 中管理 Agent Wallet，而不需要把私钥记下来、导出来，或粘贴给 Agent runtime。
...
| 用户诉求 | UnionKey 行为 |
| --- | --- |
| 创建自动化钱包 | 在 UnionKey GUI 中创建和管理 Agent Wallet。 |
...
| 端 | 职责 |
| --- | --- |
| UnionKey GUI | 创建和管理 Agent Wallet 及 keyless 账号关系 |
