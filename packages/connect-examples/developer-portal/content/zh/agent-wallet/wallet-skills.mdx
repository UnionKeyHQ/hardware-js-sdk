...
[`unionkey-wallet-skills`](https://github.com/UnionKeyHQ/unionkey-wallet-skills) 为 AI coding assistant 提供钱包能力层。用户用自然语言提需求；skills 选择安全路由，在需要时读取实时 schema，并用用户能理解的语言解释结果。
...
| Client | 安装路径 |
| --- | --- |
| Claude Code | 先 `/plugin marketplace add UnionKeyHQ/unionkey-wallet-skills`，再 `/plugin install unionkey-wallet-skills` |
| Codex | 让 Codex 按 `https://raw.githubusercontent.com/UnionKeyHQ/unionkey-wallet-skills/main/.codex/INSTALL.md` 执行 |
| Cursor | 克隆 `https://github.com/UnionKeyHQ/unionkey-wallet-skills`，并把 Cursor 指向 plugin 目录 |
...
| 自然语言 | Skill 应该做什么 |
| --- | --- |
| `Connect my UnionKey Agent Wallet.` | 通过 App Transfer 配对，并总结活动钱包。 |
