...
## Overview
An Expo screen used by the UnionKey React Native demo to exercise WalletConnect deep links. It mirrors the standalone `onekey-deeplink-demo` behaviour and plugs into the feature switcher.
...
Universal link:   https://app.unionkey.io/wc/connect/wc?uri=<WalletConnectURI>
...
- **Deep link (`onekey-wallet://...`)** calls the UnionKey app directly via the custom scheme. It requires the mobile app to be installed and launches it immediately.
- **Universal link (`https://app.unionkey.io/...`)** uses HTTPS with Apple Associated Domains / Android Digital Asset Links. When the native app is installed it will open the app; otherwise the link stays in the browser and can guide the user to download or fallback web flows.
- **Raw WalletConnect URI (`wc:...`)** lets the OS detect WalletConnect V2 handlers; UnionKey registers the scheme alongside other wallets.
...
## Use Cases
- Launch UnionKey during a WalletConnect handshake from a mobile DApp browser or another native app.
