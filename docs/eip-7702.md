...
| 攻击场景 | MetaMask 软件钱包 | UnionKey 硬件钱包 |
|---------|------------------|----------------|
...
UnionKey 硬件钱包支持以下三种经过安全审计的合约地址，以及支持进行revoke：
...
**UnionKey hardware-js-sdk**：
- **职责**：UnionKey 硬件钱包的核心 SDK
- **功能**：提供与 UnionKey 硬件设备通讯的底层接口
- **EIP-7702 实现**：处理硬件设备的 EIP-7702 交易签名

**MetaMask @metamask/accounts-monorepo**：
- **职责**：MetaMask 的账户管理核心库
- **功能**：实现各种 Keyring 接口，包括硬件钱包集成
- **EIP-7702 实现**：UnionKey Keyring 的 EIP-7702 授权签名方法
- **集成 commit**：UnionKey EIP-7702 集成实现
...
MetaMask Extension (metamask-extension)
    ↓
UnionKey Keyring (@metamask/accounts-monorepo)
    ↓
UnionKey Bridge (hardware-js-sdk)
    ↓
UnionKey 硬件设备
```
...
```typescript
// 1. 在 @metamask/accounts-monorepo 中的 UnionKey Keyring 实现
class UnionKeyKeyring {
...
**UnionKey Bridge 内部实现**：
- **UnionKeyWebBridge**：直接调用 `@unionkeyfe/hd-web-sdk`
- **UnionKeyOffscreenBridge**：通过 Chrome Extension 消息传递调用 SDK

```typescript
// UnionKey Bridge 最终调用 UnionKey hardware-js-sdk
class UnionKeyWebBridge {
  async ethereumSignTransaction(
    ...
    // 调用 UnionKey hardware-js-sdk 的核心方法
...
export const evmSignTxEip7702 = async ({
  ...
  // 构造硬件消息
  const message: EthereumSignTxEIP7702UnionKey = {
    address_n: addressN,
    to: addHexPrefix(to),
  ...
  const response = await typedCall(
    'EthereumSignTxEIP7702UnionKey',
    'EthereumTxRequestUnionKey',
    message
  );
...
## 6. 硬件安全限制与作用（UnionKey Firmware）

本节汇总 UnionKey 设备端对 EIP-7702 的主要限制与设计动机，并给出核心代码位置，便于审计与演进。
